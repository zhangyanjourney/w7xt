...
if(empty($this->settings[0]['keyword'])){
	...
	$this->settings[0]['keyword'] = $this->createMobileUrl('Gz', array('hdid'=>$_GPC['hdid']));;
...
?>
 <div id="top" class="w">
	...
	<?php 
	   $keycache = $_W['uniacid'].'-'.$_GPC['hdid'];
	   $resultcache = cache_load($keycache);
	   if(empty($resultcache)){
	   
			$touimg = tomedia($this->settings[0]['touimg']);
			$aa = @getimagesize($touimg);
			$height = $aa["1"];
	        cache_write($keycache, $height); 
	   }else{
   ...
   <div id="hdp" style="height: <?php  echo $height?>px;overflow:hidden" class="w">
	...
	<script>
        ...
        var ht = $(".hdp_ul li").height();console.log('ht：'+ht);
		if(ht==0){
		   ht=135;
		}
		$(".hdp_ul").width(zwidth);
