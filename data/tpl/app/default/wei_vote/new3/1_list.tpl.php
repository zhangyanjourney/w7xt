...
<style>
		...
		.tongji{
		   overflow:hidden;
		   margin-top:10px;padding-bottom: 10px;border-bottom: 1px solid #ddd;border-bottom: 10px solid #4b00a7;
...
body {
    background: #ffffff;
    font-size: 14px;background-color: #6f2dff;
}
...
.bg56{
background-image: url(../../addons/wei_vote/template/style/images/bg-bw.jpg);
    background-repeat: repeat-x;
    background-position: left bottom;
    background-size: 9px auto;    background-color: #6f2dff !important;padding: 10px;

}
#photo {
    padding: 10px;
    box-sizing: border-box;
    border-bottom: 10px solid #4b00a7;
}



.sd{cursor:pointer;-webkit-animation: scaleout 1.3s infinite ease-in-out;animation: scaleout 1.3s infinite ease-in-out;}
...
header, body, section, div, ul, li, p, em, a, input, button {
...
<ul class="mui-table-view">
    <li class="mui-table-view-cell mui-media">
        <a href="javascript:;">
            <img class="mui-media-object mui-pull-left" src="<?php  echo tomedia($f);?>">
            <div class="mui-media-body">
                <?php  echo $account['name'];?>
                <p class='mui-ellipsis'><?php  echo html_entity_decode($account['feifa'], ENT_QUOTES)?></p>
            </div>
        </a>
    </li>
...
<?php  if($this->settings['0']['isliwu'] == 0 || $this->settings['0']['liwuinfo'] == 1) { ?>
	<style>
...
<div class="tongji bg56">
            <ul>
                <li><i class="icon iconfont icon-renwubangdan" style="font-size: 12px;"></i>编号<label class="enroll"><?php  echo $account['bh'];?></label></li>
                <li><i class="icon iconfont icon-piaoshu" style="font-size: 14px;"></i>票数<label class="voteCount"><?php  echo $account['piao'];?></label></li>
                <li><i style="font-size: 12px;" class="icon iconfont icon-paiming"></i>排名<label class="click"><?php  echo ($z_total+1); ?></label></li>
				
				<?php  if($this->settings['0']['isliwu'] == 1&&$this->settings['0']['liwuinfo'] == 0) { ?>
					    <?php  
						  $account['yuan'] = ceil($account['yuan']); 
						?>
						<li><i style="font-size: 12px;" class="icon iconfont icon-liwu"></i>点数<label class="click"><?php  echo $account['yuan'];?></label></li>
	...
	<!--   <?php  if($this->settings['0']['yaz'] == 1) { ?>
	  ...
	  <?php  } ?> -->
				...
				<?php  if($this->settings['0']['isliwu'] == 1&&$this->settings['0']['liwuinfo'] == 0) { ?>
					...
					<div style="background: #6f2dff !important;"">
					       <div class="index_desc_title" style=" color: #fbc108"><i class="iconfont icon-liebiao"></i>礼物列表</div>
                    </div>
				<?php  } else { ?>

				    <div class ="pd" style="margin-bottom: 60px; background: #fff;background: #6f2dff !important;">
									<div class="index_desc_title" style="    color: #fbc108"><i class="iconfont icon-huodongjieshao"></i>活动介绍</div>
									<div class="desc_content" deep="2">
				...
				<?php  if(empty($liwujilu) && $this->settings['0']['isliwu'] == 1&&$this->settings['0']['liwuinfo'] == 0) { ?>
				 <div style="text-align:center;line-height: 45px;color: #fff">没有更多记录！</div>
				<?php  } ?>
				<?php  if(!empty($liwujilu)  && $this->settings['0']['isliwu'] == 1&&$this->settings['0']['liwuinfo'] == 0) { ?>			
					<ul class="mui-table-view" id="pullrefresh">
						 ...
						 <?php  if(is_array($liwujilu)) { foreach($liwujilu as $it) { ?>
							<li class="mui-table-view-cell mui-media">
								<a href="javascript:;">
									<img class="mui-media-object mui-pull-left" src="<?php  echo $it['avatar']?>">
									<div class="mui-media-body">
										<?php  echo $it['nickname']?>，给TA送了<?php  echo $it['shuliang']?>份<?php  echo $it['liwuname']?>！
										<p class='mui-ellipsis'><?php  echo date("Y-m-d H:i",$it['createtime']);?></p>
									</div>
								</a>
							</li>
						 <?php  } } ?>	
    ...
    }else
    {
		...
		if(data.c==1){
								...
								<?php  if($this->settings[0]['keyword'] == '') { ?>
							      ...
							      swal({    title: "",   text: "<div id ='guanzhu' style='width:250px;height:250px;text-align:center;margin:0px auto;'> <img src='<?php  echo $_W['account']['qrcode'];?>'><p style='text-align:center;'>请关注公众号，回复关键词<?php  echo $this->settings[0]['wxkeyword']?>进入活动</p></div>",    html: true  });
        ...
        function toupiao(k,od,inputValue){
					...
					}else
					{
							...
							<?php  if($this->settings[0]['keyword'] == '') { ?>
							      ...
							      swal({    title: "",   text: "<div id ='guanzhu' style='font-size:10px;width:250px;height:250px;text-align:center;margin:0px auto;'> <img src='<?php  echo $_W['account']['qrcode'];?>'><p style='text-align:center;'>关注公众号，回复关键词<?php  echo $this->settings[0]['wxkeyword']?>进入活动</p></div>",    html: true  });
            ...
            $(window).scroll(function() {
                ...
                if ($(document).scrollTop() >= $(document).height() - $(window).height()) {
                 if (lock)
					...
					var page = $("#page").val();
				 console.log(page);
					$.post("<?php  echo $this->createMobileUrl('Ajaxresume',array())?>", { "page": page,"id": <?php  echo $_GPC['id'];?> },
						  function(data){
					 console.log(data);
						      for(var o in data){
								...
								data[o].time = $.myTime.UnixToDate(data[o].createtime,'true');
							
								html += "<li class='mui-table-view-cell mui-media'>";
								html += "	<a href='javascript:;'>";
								html += "		<img class='mui-media-object mui-pull-left' src='"+data[o].avatar+"'>";
								html += "		<div class='mui-media-body'>";
								html += ""+data[o].nickname+"，给TA送了"+data[o].shuliang+"份"+data[o].id+"！";
								html += "			<p class='mui-ellipsis'>"+data[o].time+"</p>";
								html += "		</div>";
								html += "	</a>";
								html += "</li>";
...
<?php (!empty($this) && $this instanceof WeModuleSite) ? (include $this->template('common_foot2', TEMPLATE_INCLUDEPATH)) : (include template('common_foot2', TEMPLATE_INCLUDEPATH));?>
