	...
	<div class="remote-alioss" <?php  if(empty($remote['type']) || $remote['type'] != '2') { ?> style="display:none;" <?php  } ?>>
		...
		<div class="form-group">
		 <label class="col-sm-2 control-label">自定义URL</label>
			<div class="col-sm-9">
				<input type="text" name="custom[url]" class="form-control" <?php  if(!strexists($remote['alioss']['url'],'aliyuncs.com') && $remote['type'] == 2) { ?>value="<?php  echo $remote['alioss']['url'];?>"<?php  } ?> placeholder="默认URL不需要填写"/>
					<span class="help-block">
